...
const Skill = (props) => {
    let url=''
    if (props.title === 'JavaScript') {url=`url(${js_logo})`}
    if (props.title === 'REACT') {url=`url(${react_logo})`}
    if (props.title === 'CSS / SASS') {url=`url(${css_logo})`}
    if (props.title === 'TypeScript') {url=`url(${ts_logo})`}
    if (props.title === 'HTML') {url=`url(${html_logo})`}
    if (props.title === 'REDUX') {url=`url(${redux_logo})`}
